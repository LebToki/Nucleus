        ...
        .then(data => {
            if (data.success && data.backups) {
                ...
                backups.forEach(backup => {
                    ...
                    html += '<a href="api/backup.php?action=download_project&project=' + encodeURIComponent(backup.project) + '&timestamp=' + encodeURIComponent(backup.timestamp) + '" class="btn btn-sm btn-outline-primary-600" title="<?php echo htmlspecialchars(t_backup('download_project', 'Download Project'), ENT_QUOTES); ?>" aria-label="<?php echo htmlspecialchars(t_backup('download_project', 'Download Project'), ENT_QUOTES); ?> ' + escapeHtml(backup.project) + '"><iconify-icon icon="solar:download-bold"></iconify-icon></a>';
                    if (backup.database_zip) {
                        html += '<a href="api/backup.php?action=download_database&project=' + encodeURIComponent(backup.project) + '&timestamp=' + encodeURIComponent(backup.timestamp) + '" class="btn btn-sm btn-outline-info-600" title="<?php echo htmlspecialchars(t_backup('download_database', 'Download Database'), ENT_QUOTES); ?>" aria-label="<?php echo htmlspecialchars(t_backup('download_database', 'Download Database'), ENT_QUOTES); ?> ' + escapeHtml(backup.project) + '"><iconify-icon icon="solar:database-bold"></iconify-icon></a>';
                    }
                    html += '<button type="button" class="btn btn-sm btn-outline-warning-600" onclick="refreshBackup(\'' + escapeHtml(backup.project) + '\', \'' + escapeHtml(backup.timestamp) + '\', ' + (backup.database_zip ? '\'' + escapeHtml(backup.database || '') + '\'' : 'null') + ')" title="<?php echo htmlspecialchars(t_backup('refresh_backup', 'Refresh Backup'), ENT_QUOTES); ?>" aria-label="<?php echo htmlspecialchars(t_backup('refresh_backup', 'Refresh Backup'), ENT_QUOTES); ?> ' + escapeHtml(backup.project) + '"><iconify-icon icon="solar:refresh-bold"></iconify-icon></button>';
                    html += '<button type="button" class="btn btn-sm btn-outline-danger-600" onclick="deleteBackup(\'' + escapeHtml(backup.project) + '\', \'' + escapeHtml(backup.timestamp) + '\')" title="<?php echo htmlspecialchars(t_backup('delete_backup', 'Delete Backup'), ENT_QUOTES); ?>" aria-label="<?php echo htmlspecialchars(t_backup('delete_backup', 'Delete Backup'), ENT_QUOTES); ?> ' + escapeHtml(backup.project) + '"><iconify-icon icon="solar:trash-bin-trash-bold"></iconify-icon></button>';
                    html += '</div>';
                    html += '</td>';
                    html += '</tr>';
                });
