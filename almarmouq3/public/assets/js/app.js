(function ($) {
	...
	$(".sidebar-menu .sidebar-submenu a").on("click", function (event) {
	...
	// Keep the current page active and its submenu open.
	$(function () {
		var matched = false;

		$("ul#sidebar-menu a").each(function () {
			if (! matched && this.href === window.location.href) {
				$(this).addClass("active-page");
				$(this).closest(".sidebar-submenu").addClass("show").css("display", "block");
				$(this).closest(".dropdown").addClass("open dropdown-open");
				matched = true;
			}
		});
	});
...
*/
  ...
  */
	...
	} else { // If no button is found, just apply the current theme to the page
		updateThemeOnHtmlEl({theme: currentThemeSetting});
	}
